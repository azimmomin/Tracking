...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        # This will hold the best positions from each distribution
        bestGhosts = []
        bestPos = ()
        bestDist = float("inf")
        bestAction = None
        ...
        for action in legal:
            newPacPos = Actions.getSuccessor(pacmanPosition, action);
            cost = self.distancer.getDistance(bestPos, newPacPos)
            if cost < lowestCost:
                bestAction = action
                lowestCost = cost
        return bestAction 