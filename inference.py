    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            ...
            for nPos in newPosDist.keys():
                updatedBeliefs[nPos] += prevBelief * newPosDist[nPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while (numP > 0):
            if (numP < len(self.legalPositions)):
                particleList += self.legalPositions[0:numP - 1]
                break
            particleList += self.legalPositions
    ...
    def observe(self, observation, gameState):
        ...
        if (noisyDistance == None):
            ...
            for particle in self.particles:
                mDist = util.manhattanDistance(particle, pacmanPosition)
                updatedBeliefs[particle] += emissionModel[mDist]
        self.beliefs = updatedBeliefs
        ...
        if (self.beliefs.totalCount() > 0):
            ...
            while (i < len(self.particles)):
                nPos = util.sample(self.beliefs)
                self.particles[i] = nPos
                i += 1
        else:
            self.initializeUniformly(gameState)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legalPermutations = list(product)
        random.shuffle(legalPermutations)

        while (particleCount > 0):
            if (particleCount < len(legalPermutations)):
                self.particles += legalPermutations[0:particleCount - 1]
                break
            self.particles += legalPermutations
            particleCount -= len(legalPermutations)
        ...
        for particle in self.particles:
            ...
            for index in range(self.numGhosts):
                if noisyDistances[index] != None:
                    curEModel = emissionModels[index]
                    ghostPos = particle[index]
                    mDist = util.manhattanDistance(ghostPos, pacmanPosition)
                    p = p * curEModel[mDist]

            updatedBeliefs[particle] += p
